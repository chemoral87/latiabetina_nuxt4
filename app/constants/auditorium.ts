...
export const STATUS_COLORS: Record<string, string> = Object.keys(STATUS_CONFIG).reduce(
  (acc, key) => {
...
export const STATUS_ICONS: Record<string, string> = Object.keys(STATUS_CONFIG).reduce(
  (acc, key) => {
...
// ── Section / subsection box geometry ────────────────────────────────────────
// The framed box drawn around a seat grid. Both v1 (SeatsStageSubsection.vue)
// and v2 (SeatGrid.vue `boxed` mode) read these values so the border, the label
// bands and the stats band can never drift apart between the two mark pages.
// The box is wider/taller than the grid, which creates three bands:
//   top  (y 3 → the seat inset) → stats line (+ v1's subsection title)
//   left (x 0 → 14)             → row numbers, right-aligned
//   bottom (+43)                → column letters, below the last seat row
export const SECTION_BOX = {
  ...
  // Column letters text-top = gridHeight + EXTRA_HEIGHT - COL_LABEL_BOTTOM_GAP,
  // i.e. 9px above the box bottom edge (the box itself starts at RECT_Y).
  COL_LABEL_BOTTOM_GAP: 6,
  STATS_X: 2,
  STATS_PERCENT_X: 36,
  STATS_Y: 5,
  TITLE_X: 4,
