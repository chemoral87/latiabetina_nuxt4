...
export async function searchAddresses(
  ...
  return data
    ...
    .map((d) => {
      ...
      const residential = extractPart(d, [
        "residential",
        "suburb",
        "neighbourhood",
      ]);
...
/**
 * Build the final address string appending the city and postal code
 * when they are not already present in the address.
 */
export function buildAddressWithCity(suggestion: AddressSuggestion): string {
  const { address, city, postcode } = suggestion;
  let result = address;
