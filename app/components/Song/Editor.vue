  ...
  <VCard id="cmp-song-editor">
    ...
    <VCardText class="py-1">
      ...
      <div class="d-flex align-center flex-wrap my-3">
        <VBtn id="song-editor-paste-btn" size="small" color="primary" class="mr-2 mb-1" variant="outlined" @click="pasteDialog = true">
          <VIcon start>mdi-content-paste</VIcon>
          Pegar letra
        </VBtn>
        <VBtn id="song-editor-add-section-btn" size="small" color="primary" class="mr-2 mb-1" variant="outlined" @click="addSection">
          <VIcon start>mdi-plus</VIcon>
          Agregar sección
        </VBtn>
        <VBtn id="song-editor-add-tab-btn" size="small" color="primary" class="mr-2 mb-1" variant="outlined" @click="addTab">
          <VIcon start>mdi-plus</VIcon>
          Agregar tab
        </VBtn>
        ...
        <VBtn id="song-export-json-btn" class="mb-1" size="small" color="primary" variant="outlined" @click="exportJson">
          <VIcon start>mdi-file-download-outline</VIcon>
          Exportar JSON
        </VBtn>
        <input id="song-json-file-input" ref="jsonFileInput" type="file" style="display: none" accept=".json,application/json" @change="onJsonFileChange" />
      </div>

      <div v-if="content.sections.length === 0 && content.tabs.length === 0" class="text-center text-grey py-6">
      ...
      <div v-for="section in content.sections" :key="section.id" class="section-editor mb-3">
        ...
        <div v-for="(line, li) in section.lines" :key="line.id" class="line-editor mb-2">
          <div style="gap: 8px" class="d-flex align-center mb-1">
            <span style="min-width: 48px" class="text-caption text-grey">Línea {{ li + 1 }}</span>
            <VTextField :id="`song-line-times-${line.id}`" v-model.number="line.times" :min="1" :max="10" hide-details label="Veces" type="number" density="compact" variant="outlined" :disabled="disabled" style="max-width: 90px" title="Veces que se repite la línea" @update:model-value="(v: unknown) => { const n = Number(v); line.times = !Number.isFinite(n) || n < 1 ? 1 : Math.floor(n); }" />
          ...
          <SongEditorLineTable
            ...
            :active-syllable-id="activeSyllableId"
            :is-notes-visible="isNotesRowVisible(line)"
            @text-input="onTextInput"
            @keydown="onSyllableKeydown"
            @notes-change="onNotesChange"
            @chords-change="onChordsChange"
            @set-active="setActiveSyllable"
          />

          <div v-if="line.syllables.length === 0" class="mt-1">
            <VBtn :id="`song-line-add-syllable-btn-${line.id}`" size="x-small" variant="text" color="primary" @click="addSyllable(line)">
      ...
      <VCard v-for="tab in content.tabs" :key="tab.id" variant="tonal" class="section-editor mb-3">
        <VCardText>
          <div class="d-flex align-center mb-2">
            <VTextField :id="`song-tab-title-${tab.id}`" v-model="tab.title" label="Tab" class="mr-2" hide-details density="compact" variant="outlined" :disabled="disabled" />
            <VBtn :id="`song-tab-remove-btn-${tab.id}`" size="small" color="error" variant="text" title="Eliminar tab" @click="removeTab(tab)">
              <VIcon>mdi-delete</VIcon>
            </VBtn>
          </div>
          <VTextarea :id="`song-tab-content-${tab.id}`" v-model="tab.tablature" rows="4" auto-grow class="tab-input" density="compact" label="Tablatura" variant="outlined" :disabled="disabled" placeholder="e|--------------------------------" />
        </VCardText>
      </VCard>
    </VCardText>
...
import { useSongEditorHistory } from "~/composables/useSongEditorHistory"
import { useSongEditorNavigation } from "~/composables/useSongEditorNavigation"
...
onMounted(() => {
  ...
  if (!isEditMode.value) item.value.org_id = null
  clearErrors()
  lastSnapshotJson.value = JSON.stringify(item.value)
  nextTick(() => {
    isHistoryRestoring.value = false
...
function addSyllableAfter(line: SongLine, syllable: SongSyllable) {
  ...
  line.syllables.splice(idx === -1 ? line.syllables.length : idx + 1, 0, created)
  activeSyllableId.value = created.id
  nextTick(() => {
    activeSyllableId.value = created.id
    const el = document.querySelector("td.cell-lyric.is-active input.text-input") as HTMLInputElement | null
    el?.focus()
  })
}
...
function onChordsChange(syllable: SongSyllable, event: Event) {
  syllable.chords = parseChordNoteList((event.target as HTMLInputElement).value)
}
...
function onTextInput(syllable: SongSyllable, value: string) {
  syllable.text = value
}
...
function mergeLine(line: SongLine) {
  ...
  line.syllables[0]!.chords = [...first.chords]
  line.syllables[0]!.notes = [...first.notes]
}
function addTab() {
...
function applyPasted(text: string, mode: "replace" | "append") {
  ...
  if (mode === "replace") content.value = parsed
  else {
...
function onJsonFileChange(event: Event) {
  ...
  reader.onload = () => {
    try {
      ...
      item.value = { ...defaultSong(), ...imported, content: normalizeContent(imported.content), org_id: null, id: item.value.id } as Song
...
function exportJson() {
  ...
  const slug = (item.value.title || "cancion").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "")
  a.href = url
