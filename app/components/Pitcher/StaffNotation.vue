  ...
  <div id="cmp-pitcher-staff-notation" class="staff-notation">
    ...
    <div v-if="showCentsDeviation" class="text-right mt-2">
      <div class="text-caption">
        <strong
          id="pit-staff-cents"
          :class="tuningAccuracyClass"
          style="display: inline-block; text-align: right"
          >{{ centsDeviation > 0 ? "+" : ""
          }}{{ centsDeviation !== null ? centsDeviation : "--" }}</strong
        >
        <strong>cents</strong>
      </div>
      <VChip
        id="pit-staff-accuracy"
        size="small"
        variant="elevated"
        class="text-caption"
        :color="tuningAccuracyColor"
      >
        {{ tuningAccuracyText }}
      </VChip>
    </div>
...
const tuningMetrics = computed(() => {
  if (props.centsDeviation === null)
    return { class: "", color: "grey", text: "--" };
  const abs = Math.abs(props.centsDeviation);
  if (abs <= 5)
    return {
      class: "tuning-perfect",
      color: "green",
      text: "Perfecta afinación",
    };
  if (abs <= 15)
    return {
      class: "tuning-good",
      color: "light-green",
      text: "Buena afinación",
    };
  if (abs <= 30)
    return {
      class: "tuning-fair",
      color: "orange",
      text: "Afinación aceptable",
    };
...
function getNoteName(midiNote: number): string {
  ...
  return (latinNotation.value ? NOTE_LATIN_STRINGS : NOTE_SHORT_STRINGS)[
    fullIndex
  ];
}
...
function drawStaff() {
  ...
  const lineEnd = Math.min(
    lineStart + LINE_BASE * props.zoom,
    width - MARGIN_LINE,
  );
  ...
  const sEnd =
    noteX + NOTE_X_B_OFFSET * props.zoom + SHORT_LINE_HALF_WIDTH * props.zoom;

  for (let i = 1; i <= 2; i++)
    drawHorizontal(trebleTop - i * lineSpacing, sStart, sEnd);
  for (let i = 0; i < 5; i++)
    drawHorizontal(trebleTop + i * lineSpacing, lineStart, lineEnd);
  drawHorizontal(trebleTop + 5 * lineSpacing, sStart, sEnd);
  for (let i = 0; i < 5; i++)
    drawHorizontal(bassTop + i * lineSpacing, lineStart, lineEnd);
  for (let i = 1; i <= 2; i++)
    drawHorizontal(bassTop + 4 * lineSpacing + i * lineSpacing, sStart, sEnd);
...
function renderNotes(
  ...
  const drawPair = (
    ...
    const { noteY: ySharp, ledgerLines: lSharp } = calculateNotePos(
      midi,
      trebleTop,
      bassTop,
      lineSpacing,
    ...
    if (pairIsSharp) {
      ...
      const { noteY: yFlat, ledgerLines: lFlat } = calculateNotePos(
        flatMidi,
        trebleTop,
        bassTop,
        lineSpacing,
...
function calculateNotePos(
  ...
  bassTop: number,
  lineSpacing: number,
): { noteY: number; ledgerLines: number[] } {
  const isTreble = midi >= 60;
  const refMidi = isTreble ? 64 : 53;
  const refY = isTreble ? trebleTop + 4 * lineSpacing : bassTop + lineSpacing;
  const totalDiff =
    (Math.floor(midi / 12) - Math.floor(refMidi / 12)) * 7 +
    (NATURAL_POSITIONS[midi % 12] - NATURAL_POSITIONS[refMidi % 12]); // Use imported constant
  ...
  if (isTreble) {
    ...
    if (noteY > bottom + lineSpacing) {
      ...
      for (let i = 1; i <= count; i++)
        ledgerLines.push(bottom + lineSpacing + i * lineSpacing);
    }
  } else if (noteY < bassTop) {
    const count = Math.floor((bassTop - noteY) / lineSpacing);
    for (let i = 1; i <= count; i++)
      ledgerLines.push(bassTop - i * lineSpacing);
  }
