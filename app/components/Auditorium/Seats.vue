        ...
        <v-group v-for="(section, sIdx) in sections" :key="`section-${sIdx}`" :config="getSectionConfig(sIdx)">
          ...
          <template v-if="!section.isLabel">
            <v-group v-for="(sub, subIdx) in section.subsections" :key="`sub-${sIdx}-${subIdx}`" :config="getSubsectionPosition(section, subIdx)">
              ...
              <template v-else>
                <v-text v-for="rowIdx in getRowCount(sub)" :key="`row-label-${rowIdx}`" :config="getRowLabelConfig(rowIdx - 1)" />
...
const activeSeat = ref<{ id: string; x: number; y: number; isAbsolute?: boolean } | null>(null)

const seatSpacing = computed(() => props.settings.SEAT_SIZE + props.settings.SEATS_DISTANCE)

const tooltipPos = computed(() => {
  if (!activeSeat.value) return { x: 0, y: 0 }
  try {
    // If click handler already computed absolute tooltip position, use it
    if (activeSeat.value.isAbsolute && typeof activeSeat.value.x === "number" && typeof activeSeat.value.y === "number") {
    ...
    const x = sectionPos.x + subPos.x + activeSeat.value.x + (props.settings.SEAT_SIZE / 2 + 6)
    const y = sectionPos.y + subPos.y + activeSeat.value.y - (props.settings.SEAT_SIZE / 2 + 8)
    return { x, y }
  } catch (err) {
    return { x: activeSeat.value.x, y: activeSeat.value.y }
  }
...
const tooltipWidth = computed(() => {
  ...
  try {
    ...
    const maxText = Math.max(...categories.value.map((cat) => getTextWidth("-> " + cat.label, 12)))
...
function getSectionConfig(sIdx: number) {
  ...
  const y = props.sections.slice(0, sIdx).reduce((acc, s) => acc + getSectionHeight(s) + props.settings.SECTIONS_MARGIN, props.settings.SECTION_TOP_PADDING)
...
function getSubsectionPosition(section: Section, subIdx: number) {
  const x = section.subsections.slice(0, subIdx).reduce((acc, s) => acc + (s.isLabel ? s.width || 100 : getSubsectionWidth(s)), 0) + subIdx * props.settings.SUBSECTION_SPACING + props.settings.SECTION_SIDE_PADDING
...
function getSubsectionLabelBgConfig(sub: Subsection, section: Section) {
  return {
    width: sub.width || 100,
    height: getSectionHeight(section) - props.settings.SECTION_TOP_PADDING - props.settings.SECTION_BOTTOM_PADDING,
    fill: "#424242",
    opacity: 0.3,
    strokeWidth: 2,
...
function getSubsectionLabelTextConfig(sub: Subsection, section: Section) {
  ...
  const height = getSectionHeight(section) - props.settings.SECTION_TOP_PADDING - props.settings.SECTION_BOTTOM_PADDING
  return {
    x: width / 2,
    y: height / 2,
    text: sub.name,
...
function getSeatConfig(seat: Seat) {
  ...
  if (category) {
    ...
    try {
      const def = categories.value.find((c) => String(c.label).toLowerCase() === category || String(c.value).toLowerCase() === category)
...
function getSectionWidth(section: Section) {
  ...
  return (
    section.subsections.reduce((acc, s) => acc + (s.isLabel ? s.width || 100 : getSubsectionWidth(s)), 0) +
...
function getSectionHeight(section: Section) {
  ...
  if (!section.subsections?.length) return props.settings.SECTION_TOP_PADDING + props.settings.SECTION_BOTTOM_PADDING
  const maxRows = Math.max(...section.subsections.map((sub) => (sub.isLabel ? 0 : sub.seats?.length || 0)))
  if (maxRows === 0) return props.settings.SECTION_TOP_PADDING + props.settings.SECTION_BOTTOM_PADDING + 40
  return maxRows * seatSpacing.value - props.settings.SEATS_DISTANCE + props.settings.SECTION_TOP_PADDING + props.settings.SECTION_BOTTOM_PADDING
}
...
function handleSeatClick(seat: Seat, e: KonvaEvent) {
  ...
  try {
    ...
    if (section) {
      const sectionPos = getSectionConfig(sectionIdx)
      const subPos = getSubsectionPosition(section, subIdx)
      const absX = sectionPos.x + subPos.x + (seat.x ?? 0) + (props.settings.SEAT_SIZE / 2 + 6)
      const absY = sectionPos.y + subPos.y + (seat.y ?? 0) - (props.settings.SEAT_SIZE / 2 + 8)
      activeSeat.value = { id: seat.id, x: absX, y: absY, isAbsolute: true }
    } else {
      activeSeat.value = { id: seat.id, x: seat.x ?? 0, y: seat.y ?? 0, isAbsolute: false }
    }
  } catch (err) {
    activeSeat.value = { id: seat.id, x: seat.x ?? 0, y: seat.y ?? 0, isAbsolute: false }
  }
