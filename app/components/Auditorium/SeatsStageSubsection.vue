<template>
  <v-group id="cmp-auditorium-seats-stage-subsection" :config="{ x: 0, y: 0 }">
    ...
    <!-- Row labels -->
    <v-text v-for="rowIdx in seatRowCount" :key="`row-label-${rowIdx}`"
...
const seatSpacing = computed(() => DEFAULT_SETTINGS.SEAT_SIZE + DEFAULT_SETTINGS.SEATS_DISTANCE)
...
const subsectionWidth = computed(() => {
  ...
  const maxCols = Math.max(...seatsSource.value.map((row) => (row ? row.length : 0)))
...
const subsectionHeight = computed(() => {
  ...
  return seatsSource.value.length * seatSpacing.value - DEFAULT_SETTINGS.SEATS_DISTANCE
})
      ...
      row.forEach((seat, colIdx) => {
        if (seat && seat.state !== "invisible") {
          ...
          if ((props.loadingSeats ?? []).includes(seatId)) {
            ...
            const SEGMENTS = [
              { sweep: 30, opacity: 1.00, offset: 0, inner: radius - 1.5, outer: radius + 1.5 },
              { sweep: 40, opacity: 0.89, offset: -30, inner: radius - 1.3, outer: radius + 1.5 },
              { sweep: 50, opacity: 0.78, offset: -65, inner: radius - 1.1, outer: radius + 1.5 },
              { sweep: 55, opacity: 0.67, offset: -108, inner: radius - 0.9, outer: radius + 1.5 },
              { sweep: 55, opacity: 0.56, offset: -155, inner: radius - 0.6, outer: radius + 1.5 },
              { sweep: 50, opacity: 0.45, offset: -200, inner: radius - 0.3, outer: radius + 1.5 },
            ]
