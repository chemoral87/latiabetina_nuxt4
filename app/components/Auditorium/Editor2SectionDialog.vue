    ...
    <VCard v-if="section" id="ae2-section-edit-card">
      ...
      <VCardText class="pt-0">
        ...
        <VRow class="mb-3" density="compact">
          ...
          <VCol sm="2" cols="4" class="d-flex align-center">
            <VBtn
              id="ae2-section-grid-btn"
              size="small"
              color="primary"
              variant="elevated"
              @click="applyGrid"
...
const miniStageConfig = computed(() => {
  ...
  return {
    ...
    height: Math.max(
      120,
      getFloatingSectionHeight(s) + GRID_PAD_Y + GRID_PAD_BOTTOM,
    ),
  }
})
...
const tooltipWidth = computed(() => {
  ...
  try {
    const maxText = Math.max(
      ...categories.map((cat) => getTextWidth("-> " + cat.label, 12)),
    )
...
function applyGrid() {
  ...
  resizeFloatingSectionGrid(props.section, tempRows.value, tempCols.value)
