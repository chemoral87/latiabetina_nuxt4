<template>
  <VSheet id="ae2-mark-canvas-sheet" color="black" elevation="2" class="pa-0 ma-0 stage-container" :style="{ height: '100%', width: '100%', overflow: 'hidden' }">
    <VStage
      ref="konvaStage"
    ...
    >
      ...
      >
        ...
        <VGroup v-for="tag in visibleTags" :key="`tag-${tag.id}`" :config="{ x: tag.x, y: tag.y }">
      ...
      <VLayer v-if="tooltip.visible">
        <VGroup :config="{ x: tooltip.x, y: tooltip.y }">
          <VRect :config="{ width: 50, height: 20, fill: '#222', opacity: 0.9 }" />
          <VText :config="{ text: tooltip.text, fontSize: 12, fill: '#fff', fontFamily: 'Arial', padding: 4 }" />
        </VGroup>
      </VLayer>
    </VStage>
  </VSheet>
...
<script setup lang="ts">
  import { COLORS, DEFAULT_SETTINGS, FLOATING_SECTION_BOX, STAGE_CATEGORIES, SUBSECTION_BORDER_COLORS, SUBSECTION_BORDER_WIDTH } from '~/constants/auditorium'
  ...
  const { konvaStage, zoomLevel, minZoom, maxZoom, isTwoFingerGesture, handleWheel, handleTouchStart, handleTouchMove, handleTouchEnd, handleDragStart, handleDragEnd } =
  ...
  function showTooltip(e: any, text: string) {
    ...
    try {
      ...
      if (!pos) return
      tooltip.x = pos.x / zoomLevel.value - 290
      tooltip.y = pos.y / zoomLevel.value - 72
      tooltip.text = text
  ...
  const visibleTags = computed<FloatingTag[]>(() => (props.selectedSectionId ? [] : (props.config.tags ?? [])))
  ...
  function getContentBBox() {
    ...
    for (const s of sections) {
      ...
      const h = s.rows * seatSpacing - seatsDistance + FLOATING_SECTION_BOX.EXTRA_HEIGHT + FLOATING_SECTION_BOX.RECT_Y
