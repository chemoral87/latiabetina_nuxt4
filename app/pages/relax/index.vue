<template>
  <VContainer :fluid="true" class="breathing-container">
      ...
      <VCol cols="12">
        <VCard id="rel-animation-card" class="pa-3" rounded="lg" elevation="3" color="grey-lighten-5">
          <VRow align="center" density="comfortable">
            <!-- Botón de control -->
            <VCol cols="5">
              <VBtn id="rel-toggle-btn" rounded size="small" elevation="2" :color="isPlaying ? 'error' : 'primary'" @click="toggleAnimation">
            ...
            <VCol cols="7" class="d-flex flex-wrap gap-1">
              ...
              >
                ...
                Contracción inicial
                <span v-if="animationState === 'initialContract'" class="rel-countdown">{{ stepRemainingDisplay }}</span>
              </VChip>
              <VChip
                v-if="expansion > 0"
              ...
              >
                ...
                Expansión
                <span v-if="animationState === 'expansion'" class="rel-countdown">{{ stepRemainingDisplay }}</span>
              </VChip>
              <VChip
                v-if="immobile1 > 0"
              ...
              >
                ...
                Inmóvil 1
                <span v-if="animationState === 'immobile1'" class="rel-countdown">{{ stepRemainingDisplay }}</span>
              </VChip>
              <VChip
                v-if="contraction > 0"
              ...
              >
                <VIcon start size="x-small">mdi-arrow-collapse-all</VIcon>
                Contracción
                <span v-if="animationState === 'contraction'" class="rel-countdown">{{ stepRemainingDisplay }}</span>
              </VChip>
              <VChip
                v-if="immobile2 > 0"
              ...
              >
                <VIcon start size="x-small">mdi-timer-sand</VIcon>
                Inmóvil 2
                <span v-if="animationState === 'immobile2'" class="rel-countdown">{{ stepRemainingDisplay }}</span>
              </VChip>
            </VCol>
      ...
      <VCol cols="12">
        <VCard id="rel-config-card" class="pa-1" rounded="lg" elevation="2">
          ...
          <VRow density="comfortable">
            <VCol cols="auto">
              <VTextField id="rel-initial-contract" v-model.number="initialContract" step="0.1" suffix="s" hide-details type="number" density="compact" variant="outlined" :disabled="isPlaying" color="orange-darken-2" label="Contracción inicial">
                <template #prepend-inner>
                  <VIcon size="small" color="orange-darken-2">mdi-arrow-collapse-all</VIcon>
                </template>
              </VTextField>
            </VCol>

            <VCol cols="auto">
              <VTextField id="rel-expansion" v-model.number="expansion" step="0.1" suffix="s" color="blue" hide-details type="number" density="compact" label="Expansión" variant="outlined" :disabled="isPlaying">
                <template #prepend-inner>
            ...
            <VCol cols="auto">
              <VTextField id="rel-immobile-1" v-model.number="immobile1" step="0.1" suffix="s" hide-details color="green" type="number" density="compact" label="Inmóvil 1" variant="outlined" :disabled="isPlaying">
                <template #prepend-inner>
            ...
            <VCol cols="auto">
              <VTextField id="rel-contraction" v-model.number="contraction" step="0.1" suffix="s" color="red" hide-details type="number" density="compact" variant="outlined" label="Contracción" :disabled="isPlaying">
                <template #prepend-inner>
            ...
            <VCol cols="auto">
              <VTextField id="rel-immobile-2" v-model.number="immobile2" step="0.1" suffix="s" hide-details color="green" type="number" density="compact" label="Inmóvil 2" variant="outlined" :disabled="isPlaying">
                <template #prepend-inner>
                  <VIcon size="small" color="green">mdi-timer-sand</VIcon>
                </template>
              </VTextField>
            ...
            <VCol cols="auto">
              <VTextField id="rel-goal-time" v-model.number="goalTime" step="1" hide-details suffix="min" type="number" density="compact" variant="outlined" :disabled="isPlaying" color="purple-darken-2" label="Tiempo objetivo">
                <template #prepend-inner>
                  <VIcon size="small" color="purple-darken-2">mdi-flag-checkered</VIcon>
                </template>
              </VTextField>
            </VCol>
          </VRow>
    ...
    <VDialog id="rel-completion-dlg" v-model="showCompletionDialog" persistent max-width="340">
      <VCard id="rel-completion-card" rounded="xl" class="text-center pa-4">
        <div class="celebration-icon mb-2">🎉</div>
        <VCardTitle class="text-h6 justify-center pb-1">¡Felicidades!</VCardTitle>
        <VCardText class="text-body-1 pb-2">
          Completaste la rutina de <strong>{{ goalTime }} minuto{{ goalTime !== 1 ? 's' : '' }}</strong>.<br/>
          ¡Excelente trabajo!
        </VCardText>
        <div class="d-flex justify-center">
          <VBtn id="rel-close-btn" rounded elevation="2" color="purple-darken-2" @click="closeCompletion">
...
const exerciseMap: Record<string, BreathingExercise> = {
  ...
  military: { initialContract: 0, expansion: 4, immobile1: 7, contraction: 8, immobile2: 0 },
}
...
function getPhaseDuration(state: string): number {
  switch (state) {
    case "initialContract": return initialContract.value
    case "expansion": return expansion.value
    case "immobile1": return immobile1.value
    case "contraction": return contraction.value
    case "immobile2": return immobile2.value
    default: return 0
  }
}
